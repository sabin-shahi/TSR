...
def get_coordinates(request):
    if request.method == 'POST':
        ...
        api_key = 'AlzaSyGmYnQ8aLg__TzaEEMLUIoaMqNH1dnZJAD'  # Replace with your own API key
        ...
        try:
            ...
            return JsonResponse({'error': 'Failed to retrieve coordinates'}, status=500)
    return HttpResponse('Enter an address to get coordinates')

# def get_coordinates(request):
#     if request.method == 'POST':
#         address = request.POST.get('address')
#         api_key = '362a7487baa25af221c3d5ca35bd1212a9f9312b'  # Replace with your own API key
#         url = f'https://maps.googleapis.com/maps/api/geocode/json?address={address}&key={api_key}'
#         response = requests.get(url)
#         data = response.json()
#         if data['status'] == 'OK':
